...
module.exports = {
  ...
  async up (queryInterface, Sequelize) {
    ...
    */
    ...
    await queryInterface.bulkDelete('order_products', null, { restartIdentity: true, truncate: true, cascade: true });
    await queryInterface.bulkDelete('orders', null, { restartIdentity: true, truncate: true, cascade: true });

    const usersList = await queryInterface.rawSelect('users',{ where: {}, plain: false }, ['id']);
    
    for(const userItem of usersList) {
      const totalOrders = this.randomNumber(1,20);
      const userAddress = await queryInterface.rawSelect('address', { where: { user_id: userItem.id },plain: false }, ['id']);
      ...
      for(let i=0; i < totalOrders; i++) {
        ...
        await queryInterface.bulkUpdate('orders',{ amount: amount, net_amount: amount + 10 }, { id: orderDetails[0].id });
      }
    }
  },
